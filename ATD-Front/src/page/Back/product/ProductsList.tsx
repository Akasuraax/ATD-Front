...
function ProductsList(){
    ...
    const columns: GridColDef[] = [
        ...
        {
            field: 'name',
            headerName: t("product.titled"),
            width: 350,
            editable: false,
        },
        {
            field: 'measure',
            headerName: t("product.measure"),
            width: 200,
            editable: false,
        },
    ];
    ...
    async function sendRequest() {
        ...
        try {
            ...
            setRowCount(productsResponse.total)
            setStandBy(false);
        } catch (error) {
            setStandBy(false);
        }
    ...
    return(
        <main>
            <div className="m-auto content max-w-screen-xl">
                <h2 className="text-center mb-8">{t('product.title')}</h2>
                <button
                    type="button"
                    onClick={() => {
                        navigate(`/back/products/add`)
                    }}
                ...
                <Box sx={{height: "500px", width: 'auto'}}>
                    <DataGrid
                        ...
                        loading={standBy}
                        onRowClick={(params) => {
                            navigate(`/back/products/${params.row.id}`)
                        }}
                    />
                </Box>
            </div>
